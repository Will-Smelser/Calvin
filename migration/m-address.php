...
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    $q = "INSERT INTO {$mysqldb}country_codes VALUES (NULL,'$c','{$row['Country Abreviation']}')";
    $res = mysql_query($q);
...
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    $q = "INSERT INTO {$mysqldb}state_codes VALUES (NULL,'{$row['State Abbreviation']}','{$row['State']}')";
    $res = mysql_query($q);
...
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    $q = "insert into {$mysqldb}address values (NULL,\"{$row['Company Address']}\",NULL,\"{$row['Company City']}\",$scode,\"{$row['Company Zip Code']}\",$ccode)";
    $res = mysql_query($q);
    if(!$res){
        echo "<span style='color:red'>FAILED: </span>$q<br/>\n";
        echo mysql_error();
    ...
    $q = "insert into {$mysqldb}company values (NULL,$id,\"$abr\",$active,\"$name\",\"$rep\",\"$phone\",\"$web\")";
    $res = mysql_query($q);
    if(!$res){
        echo "<span style='color:red'>FAILED: </span>$q<br/>\n";
        echo mysql_error()."<br/>";
...
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    $q = "insert into {$mysqldb}facility values (NULL,$cid,$id";
